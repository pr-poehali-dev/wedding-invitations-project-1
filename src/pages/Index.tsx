...
// Golden Brown (Slowed) — YouTube ID
const YT_VIDEO_ID = "7UVm0mPCRR0";

// Хук для scroll-анимаций
...
const TimingRow = ({ time, title, sub, last }: { time: string; title: string; sub?: string; last?: boolean }) => (
  <div className="reveal text-center py-5" style={{ borderBottom: last ? "none" : "1px solid rgba(123,28,46,0.12)" }}>
...
export default function Index() {
  ...
  // Управление YouTube через postMessage
  const toggleMusic = () => {
    if (!playerRef.current) return;
    const cmd = playing
      ? '{"event":"command","func":"pauseVideo","args":[]}'
      : '{"event":"command","func":"playVideo","args":[]}';
    playerRef.current.contentWindow?.postMessage(cmd, "*");
    setPlaying(!playing);
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "fixed", width: 1, height: 1, overflow: "hidden", opacity: 0, pointerEvents: "none", top: 0, left: 0, zIndex: -1 }}>
        <iframe
          ref={playerRef}
          id="yt-player"
          src={`https://www.youtube.com/embed/${YT_VIDEO_ID}?enablejsapi=1&loop=1&playlist=${YT_VIDEO_ID}&autoplay=0`}
          allow="autoplay"
        ...
        {playing && (
          <div className="flex gap-1 items-end" style={{ height: 16 }}>
            {[0.4, 0.8, 0.5, 1, 0.6].map((h, i) => (
              <div key={i} style={{ width: 3, height: `${h * 16}px`, background: "rgba(255,255,255,0.6)", borderRadius: 2, animation: `bounce ${0.5 + i * 0.12}s ease-in-out infinite alternate` }} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
        ...
        {/* === COLUMN 1 === */}
        <div className="reveal inv-card flex flex-col" style={{ background: "#f5f0eb" }}>
          ...
          <div className="px-4 mb-2" style={{ position: "relative" }}>
            <img src={IMG_ENVELOPE} alt="Приглашение" className="w-full rounded-sm"
              style={{ objectFit: "cover", maxHeight: 280, boxShadow: "0 8px 32px rgba(107,20,34,0.3)" }} />
            <div style={{ position: "absolute", top: "22%", left: 0, right: 0, textAlign: "center", pointerEvents: "none" }}>
              <p className="font-script" style={{ color: "#5a1220", fontSize: "clamp(2.4rem, 9vw, 3.4rem)", lineHeight: 1.1, textShadow: "0 2px 12px rgba(255,255,255,0.95), 0 0 30px rgba(255,255,255,0.7)" }}>
                Евгений
              </p>
              <p style={{ fontFamily: "'Cormorant Garamond', serif", color: "#7B1C2E", fontSize: "clamp(1.2rem, 4vw, 1.8rem)", letterSpacing: "0.4em", textShadow: "0 1px 8px rgba(255,255,255,0.95)", margin: "2px 0" }}>
                &amp;
              </p>
              <p className="font-script" style={{ color: "#5a1220", fontSize: "clamp(2.4rem, 9vw, 3.4rem)", lineHeight: 1.1, textShadow: "0 2px 12px rgba(255,255,255,0.95), 0 0 30px rgba(255,255,255,0.7)" }}>
                София
          ...
          {/* Save the date */}
          <div className="reveal mx-6 my-4 px-5 py-5 text-center" style={{ border: "1.5px solid #7B1C2E" }}>
            <p className="font-sans-inv uppercase tracking-[0.2em]" style={{ fontSize: "0.62rem", color: "#2a1a1a", fontWeight: 600 }}>Save the date</p>
            <p className="font-cormorant mt-1" style={{ fontSize: "2.4rem", color: "#7B1C2E", fontWeight: 300, fontStyle: "italic" }}>22 / 07 / 26</p>
          </div>

          <div className="reveal flex justify-center my-2">
        ...
        <div className="reveal inv-card flex flex-col" style={{ background: "#f5f0eb" }}>
          ...
          <div className="reveal mx-5 rounded-sm overflow-hidden" style={{ border: "1px solid rgba(123,28,46,0.2)" }}>
            <div style={{ background: "#ede8e0", padding: "12px 14px 6px", textAlign: "center", fontSize: "1.2rem" }}>🌸 🌺 🌸</div>
            ...
            <div className="overflow-hidden" style={{ height: 160 }}>
              <img src={IMG_VENUE} alt="Аврора" className="w-full h-full object-cover" style={{ objectPosition: "center top" }}
                onError={e => { (e.target as HTMLImageElement).parentElement!.style.background = "#d4c5b5"; (e.target as HTMLImageElement).style.display = "none"; }} />
            </div>
            <div style={{ background: "#ede8e0", padding: "6px 14px 12px", textAlign: "center", fontSize: "1.2rem" }}>🌺 🌸 🌺</div>
          </div>
        ...
        <div className="reveal inv-card flex flex-col" style={{ background: "#f5f0eb" }}>
          ...
          <div className="reveal flex justify-center gap-3 mb-4 px-6">
            {[{ bg: "#6B1422" }, { bg: "#e8ddd0", border: "#ccc" }, { bg: "#c4879a" }, { bg: "#d4c5b5", border: "#bbb" }].map((c, i) => (
          ...
          <div className="flex justify-center my-2" style={{ fontSize: "3.5rem" }}>☀️</div>

          <div className="reveal px-7 pt-4 pb-3 text-center">
          ...
          {!rsvpSent ? (
            <div className="px-6 pb-6 flex flex-col gap-3">
              <input type="text" placeholder="Имя Фамилия" value={name} onChange={e => setName(e.target.value)}
                ...
                style={{ border: "1px solid rgba(123,28,46,0.3)", borderRadius: 2, background: "#fff", fontFamily: "'Montserrat', sans-serif", fontSize: "0.75rem", color: "#2a1a1a" }} />
              ...
              <button className="rsvp-btn w-full mt-1" onClick={handleRsvp}>Подтвердить во ВКонтакте</button>
              <p className="text-center font-sans-inv" style={{ fontSize: "0.6rem", color: "#aaa" }}>Ответ откроется в ВКонтакте</p>
            </div>
          ) : (
            <div className="px-6 pb-8 text-center">
              <div style={{ fontSize: "2rem", marginBottom: 8 }}>🎀</div>
              <Script size="2rem" color="#7B1C2E">Будем вас ждать!</Script>
              <p className="font-cormorant mt-2" style={{ color: "#888", fontStyle: "italic", fontSize: "0.9rem" }}>До встречи на торжестве ♡</p>
            </div>
          )}

          {!rsvpSent && (
            <div className="reveal px-6 pb-8 text-center">
      ...
      <div className="reveal mt-8 text-center">
        <p className="font-script" style={{ color: "rgba(255,255,255,0.5)", fontSize: "1.4rem" }}>Евгений & София · 22 июля 2026</p>
      </div>
    </div>
  );
}
